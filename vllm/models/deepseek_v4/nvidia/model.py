...
class MegaMoEBackend:
    ...
    :func:`_get_mega_moe_backend`: obtaining a backend already implies that
    the current device and shapes are supported.
    """
    ...
    def supports_shared_experts(self, deep_gemm) -> bool:
        """Whether the runtime deep_gemm supports fused shared experts.

        Default backends do not fuse shared experts. Subclasses override this
        to probe the installed deep_gemm build's Python API.
        """
        return False
    ...
    def transform_shared_expert_weights(
        ...
        *,
        ...
        A ``None`` return means the backend cannot fuse shared experts for
        this module; the caller should fall back to a serial shared MLP.
        """
        return None
...
class DeepGemmSm100MegaMoEBackend(MegaMoEBackend):
    ...
    def transform_weights(
        ...
        *,
        ...
        w13_scale = deep_gemm.transform_sf_into_required_layout(
            _ue8m0_uint8_to_float(w13_weight_scale).contiguous(),
            2 * intermediate_size,
            hidden_size,
            (1, 32),
            num_local_experts,
        ...
        w2_scale = deep_gemm.transform_sf_into_required_layout(
            _ue8m0_uint8_to_float(w2_weight_scale).contiguous(),
            hidden_size,
            intermediate_size,
            (1, 32),
            num_local_experts,
    ...
    def _prepare_shared_expert_scale(
        ...
        scale_fp32 = _ue8m0_uint8_to_float(scale.view(torch.uint8))
        scale_1x32 = (
...
def _get_mega_moe_backend(
    device: torch.device,
    hidden_size: int,
    intermediate_size: int,
) -> MegaMoEBackend:
    """Return the MegaMoE backend for ``device`` after validating support.
    ...
    on the returned backend. Raises :class:`NotImplementedError` or
    :class:`ValueError` if the device / shapes are unsupported.

    Currently only the SM100 deep_gemm backend is registered here. SM90 and
    other backends are expected to be provided by plugins that override this
    factory.
    """
...
class DeepseekV4MegaMoEExperts(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Selected MegaMoE backend. Populated lazily via _ensure_backend() so
        # that construction time (which happens before weights are placed on
        # device) does not need to know the target GPU architecture.
        self._backend: MegaMoEBackend | None = None
    ...
    def _ensure_backend(self) -> MegaMoEBackend:
